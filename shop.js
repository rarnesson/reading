...
const firebaseConfig = {
  ...
  // 🎯 Klassmål
  const GOAL1 = 3000; // Roblox/Minecraft
  const GOAL2 = 6000; // Fika
  ...
  function updateClassUI() {
    ...
    if (currentClassPoints <= GOAL1) {
      // Första etappen: 0 → 4000
      pct = (currentClassPoints / GOAL1) * 100;
    } else if (currentClassPoints <= GOAL2) {
      // Andra etappen: 4000 → 8000 (ny “mätare” 0–100)
      const etapppoang = currentClassPoints - GOAL1;
      pct = (etapppoang / (GOAL2 - GOAL1)) * 100;
    } else {
      pct = 100;
    }
  ...
  async function handleBuy(item, button) {
    ...
    try {
      ...
      await db.runTransaction(async (t) => {
        ...
        const current = typeof data.poang === "number" ? data.poang : 0;
